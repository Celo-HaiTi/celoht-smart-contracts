...
export const USDM_ADDRESS_CELO_VERIFIED = "0x765DE816845861e75A25fCA122bb6898B8B1282a";
...
export const USDM_ADDRESS_ALFAJORES_VERIFIED = "0x874069Fa1Eb16D44d622F2e0Ca25eeA172369bC1";
...
export const CONFIRMED_CELOHT_TREASURY = "0xd856e0599cc49C9cef6C358d2c2f064112A6b384";
...
function requireValidAddress(name: string, value: string): string {
  ...
  if (checksummed === getAddress("0x0000000000000000000000000000000000000000")) {
...
export function loadDeploymentConfig(networkName: string): DeploymentConfig {
  const isRealNetwork = networkName === "celo" || networkName === "alfajores" || networkName === "celoSepolia";

  let usdm: string;
  if (networkName === "celo") {
    usdm = requireValidAddress("USDM_ADDRESS_CELO", requireEnv("USDM_ADDRESS_CELO"));
  } else if (networkName === "alfajores") {
    usdm = requireValidAddress("USDM_ADDRESS_ALFAJORES", requireEnv("USDM_ADDRESS_ALFAJORES"));
    ...
    usdm = requireValidAddress("USDM_ADDRESS_CELO_SEPOLIA", requireEnv("USDM_ADDRESS_CELO_SEPOLIA"));
  } else {
  ...
  const generalTreasury = requireValidAddress("GENERAL_TREASURY", requireEnv("GENERAL_TREASURY"));
  const educationTreasury = requireValidAddress("EDUCATION_TREASURY", requireEnv("EDUCATION_TREASURY"));
  const reforestationTreasury = requireValidAddress(
    "REFORESTATION_TREASURY",
    requireEnv("REFORESTATION_TREASURY")
  );
  const governanceTreasury = requireValidAddress("GOVERNANCE_TREASURY", requireEnv("GOVERNANCE_TREASURY"));

  return { usdm, generalTreasury, educationTreasury, reforestationTreasury, governanceTreasury };
}
