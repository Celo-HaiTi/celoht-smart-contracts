...
describe("CeloHTReforestation", () => {
  async function deployFixture() {
    ...
    const Reforestation = await ethers.getContractFactory(
      "CeloHTReforestation",
    );
  ...
  it("accepts a voluntary USDm donation and routes it to the treasury", async () => {
    const { treasury, donor1, usdm, reforestation } =
      await loadFixture(deployFixture);
    const amount = ethers.parseUnits("5", 18);
    await fundAndApprove(
      usdm,
      donor1,
      await reforestation.getAddress(),
      amount,
    );
  ...
  it("rejects a zero-amount donation", async () => {
    ...
    await expect(
      reforestation.connect(donor1).donate(0),
    ).to.be.revertedWithCustomError(reforestation, "ZeroAmount");
  });

  it("tracks cumulative totals across multiple donors", async () => {
    const { donor1, donor2, usdm, reforestation } =
      await loadFixture(deployFixture);
  ...
  it("rejects invalid donation IDs", async () => {
    const { reforestation } = await loadFixture(deployFixture);
    await expect(reforestation.getDonation(999)).to.be.revertedWithCustomError(
      reforestation,
      "DonationNotFound",
    );
  });
  ...
  it("only TREASURY_ADMIN_ROLE can update the treasury, and rejects zero address", async () => {
    ...
    await expect(reforestation.connect(donor1).setTreasury(donor1.address)).to
      .be.reverted;
    await expect(
      reforestation.connect(admin).setTreasury(ethers.ZeroAddress),
    ).to.be.revertedWithCustomError(reforestation, "ZeroAddress");
  });
});
