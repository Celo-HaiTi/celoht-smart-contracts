...
describe("CeloHTServicePayments", () => {
  async function deployFixture() {
    const [admin, treasury, agentWallet, customer, other] =
  ...
  it("enforces the exact 80/20 split with no funds created or destroyed (P2P)", async () => {
    const { treasury, agentWallet, customer, usdm, payments, agentId } =
  ...
  it("enforces the exact 80/20 split for Education service payments", async () => {
    ...
    await payments
      .connect(customer)
      .payForService(ServiceType.EDUCATION, agentId);
  ...
  it("reforestation assistance is free and moves zero funds", async () => {
    ...
    await expect(
      payments
        .connect(customer)
        .payForService(ServiceType.REFORESTATION, agentId),
    )
  ...
  it("rejects payment to a nonexistent agent", async () => {
    ...
    await fundAndApprove(usdm, customer, await payments.getAddress(), FEES.p2p);
    await expect(
      payments.connect(customer).payForService(ServiceType.P2P, 999),
    ).to.be.revertedWithCustomError(payments, "AgentNotFound");
  });

  it("rejects paid-service revenue for an inactive agent", async () => {
    const { admin, customer, usdm, registry, payments, agentId } =
      await loadFixture(deployFixture);
    await registry.connect(admin).setAgentStatus(agentId, false);
    await fundAndApprove(usdm, customer, await payments.getAddress(), FEES.p2p);

    await expect(
      payments.connect(customer).payForService(ServiceType.P2P, agentId),
    ).to.be.revertedWithCustomError(payments, "AgentInactive");
  });
  ...
  it("records unique, queryable payment IDs", async () => {
    const { customer, usdm, payments, agentId } =
      await loadFixture(deployFixture);
    await fundAndApprove(
      usdm,
      customer,
  ...
  it("rejects invalid payment IDs", async () => {
    ...
    await expect(payments.getPayment(999)).to.be.revertedWithCustomError(
      payments,
      "PaymentNotFound",
    );
  });
  ...
  it("only FEE_ADMIN_ROLE can update prices, and rejects the wrong split configuration", async () => {
    ...
    await expect(payments.connect(other).setServicePrice(ServiceType.P2P, 1)).to
      .be.reverted;
    await expect(
      payments.connect(admin).setServicePrice(ServiceType.P2P, 1),
    ).to.emit(payments, "ServicePriceUpdated");

    await expect(payments.connect(other).setAgentShareBps(9000)).to.be.reverted;
    await expect(
      payments.connect(admin).setAgentShareBps(10001),
    ).to.be.revertedWithCustomError(payments, "InvalidBps");
  });
});
