...
contract CeloHTAgentRegistry is ICeloHTAgentRegistry, AccessControl {
    ...
    constructor(address usdmToken, address initialTreasury, uint256 initialRegistrationFee, address admin) {
    ...
    function registerAgent() external returns (uint256 agentId) {
        if (_agentIdByWallet[msg.sender] != 0) revert AlreadyRegistered(msg.sender);
        ...
        if (registrationFee > 0) {
            usdm.safeTransferFrom(msg.sender, treasury, registrationFee);
        }

        emit AgentRegistered(agentId, msg.sender, uint64(block.timestamp));
    }

    function setAgentStatus(uint256 agentId, bool active) external onlyRole(AGENT_ADMIN_ROLE) {
    ...
    function setAgentVerified(uint256 agentId, bool verified) external onlyRole(AGENT_ADMIN_ROLE) {
        Agent storage agent = _agents[agentId];
        if (agent.wallet == address(0)) revert AgentNotFound(agentId);
    ...
    function setRegistrationFee(uint256 newFee) external onlyRole(AGENT_ADMIN_ROLE) {
